...
SELECT 
	o.employee_id, e.first_name, e.last_name, COUNT(o.order_id) AS Total_Sales,
	RANK() OVER(
...
SELECT 
	...
	order_date,
	freight,
	LEAD(freight) OVER(PARTITION BY customer_id ORDER BY  order_date) AS previous_freight,
	LAG(freight) OVER(PARTITION BY customer_id ORDER BY order_date) AS next_freight
FROM 
orders;
...
COUNT(*) AS product_count,ROUND(AVG(unit_price)::numeric, 2) AS avg_price
FROM price_cte
